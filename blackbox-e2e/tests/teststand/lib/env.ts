...
export const config = {
  ...
  startApScript: envStr(
    "TESTSTAND_START_AP_SCRIPT",
    "firmware/tests/real_hardware/start-ap.sh",
  ),
  ...
  reconnectTimeoutMs: Number(
    envStr("TESTSTAND_RECONNECT_TIMEOUT_MS", "120000"),
  ),
  // Boot with no AP: WiFi boot attempts (~10s + LED pulses) fail, the reader
  // loop starts and runtime maintenance takes over.
  bootOfflineGraceMs: Number(
    envStr("TESTSTAND_BOOT_OFFLINE_GRACE_MS", "25000"),
  ),
} as const;
