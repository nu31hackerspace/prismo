...
export async function watchDeviceStatus(credentials: MqttCredentials): Promise<StatusWatcher> {
	...
	client.on('message', (t, payload) => {
		...
		try {
			...
			samples.push({ receivedAt: Date.now(), online: !!data.online, uptimeS: data.uptime_s });
		} catch {
	...
	await new Promise<void>((resolve, reject) => {
		...
		client.on('error', (err) => {
			...
			console.warn(`status-watcher mqtt error (will auto-reconnect): ${err.message}`);
		});
		client.once('connect', () => {
	...
	return {
		...
		async waitForSample(afterMs: number, timeoutMs: number): Promise<StatusSample> {
