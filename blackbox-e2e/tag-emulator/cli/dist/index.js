...
program
  ...
  .requiredOption(
    "-p, --port <port>",
    "Serial port path (e.g., /dev/tty.usbmodem1101 or /dev/ttyACM0)",
  )
  .requiredOption(
    "-k, --key <hex>",
    "6-character hex string of the NFC key to simulate (e.g., aabbcc)",
  )
  .requiredOption(
    "-t, --time <seconds>",
    "Time in seconds to simulate the key",
    parseFloat,
  )
  .action((options) => {
    ...
    const parser = port.pipe(
      new parser_readline_1.ReadlineParser({ delimiter: "\n" }),
    );
    port.on("open", () => {
      console.log(
        `Connected! Offloading emulation task for ${time} seconds to the device...`,
      );
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          port.close();
          console.log(
            `Command sent: ${key} for ${time}s. CLI is exiting now; the device will handle the timer.`,
          );
          process.exit(0);
        }, 100);
      }, 500);
