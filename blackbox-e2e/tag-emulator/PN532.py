...
# Target (card emulation) commands
...
class PN532:
    ...
    def tg_init_as_target(self, uid, timeout=2000):
        """Configure the PN532 as a passive ISO14443-A target (card emulation).

        ``uid`` is a 3-byte value used as the NFCID1 of the emulated tag. An
        external reader doing InListPassiveTarget will detect this tag; the UID
        it sees is 4 bytes, prefixed by 0x08 (the "random UID" cascade tag),
        followed by these 3 bytes.

        Blocks until an external reader activates the target or ``timeout``
        milliseconds elapse. Returns the activation data (Mode byte + initiator
        command) when activated, or None on timeout.
        """
        assert len(uid) == 3, 'NFCID1 (uid) must be exactly 3 bytes'
        params = bytearray()
        # Mode: 0x05 = PICC only (0x04) + Passive only (0x01)
        params.append(0x05)
        # --- MIFARE params (6 bytes) ---
        params += bytes([0x04, 0x00])     # SENS_RES (ATQA)
        params += bytes(uid)              # NFCID1t (3 bytes)
        params.append(0x08)               # SEL_RES (SAK) -> MiFare-like
        # --- FeliCa params (18 bytes) ---
        params += bytes([0x01, 0xFE, 0xA2, 0xA3, 0xA4, 0xA5, 0xA6, 0xA7])  # NFCID2t
        params += bytes([0xC0, 0xC1, 0xC2, 0xC3, 0xC4, 0xC5, 0xC6, 0xC7])  # PAD
        params += bytes([0xFF, 0xFF])     # System code
        # --- NFCID3t (10 bytes) ---
        params += bytes([0xAA, 0x99, 0x88, 0x77, 0x66, 0x55, 0x44, 0x33, 0x22, 0x11])
        # --- General bytes (0) and Historical bytes (0) ---
        params.append(0x00)               # LENGb
        params.append(0x00)               # LENGt
        return self.call_function(_COMMAND_TGINITASTARGET,
                                  params=list(params),
                                  response_length=64,
                                  timeout=timeout)
