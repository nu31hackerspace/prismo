<script lang="ts">
	...
	const flashCallbacks = {
		onStateChange: (s: FlasherState) => { flasherState = s; },
		onLog: (log: FlasherLog) => { flashLogs = [...flashLogs, log]; scrollLogs(); },
		onProgress: (pct: number) => { flashProgress = pct; },
		onChipInfo: (info: FlasherInfo) => { flashChipInfo = info; },
		onError: (msg: string) => { flashError = msg; }
	};
	...
	$effect(() => {
		...
		function onDisconnect() { if (flasherState === 'unplug') flasherState = 'replug'; }
		function onConnect() { if (flasherState === 'replug' && replugCountdown <= 0) flasherState = 'booting'; }
	...
	$effect(() => {
		...
		replugCountdown = 10;
		const iv = setInterval(() => { replugCountdown -= 1; if (replugCountdown <= 0) clearInterval(iv); }, 1000);
	...
	$effect(() => {
		...
		const iv = setInterval(() => {
			bootCountdown -= 1;
			if (bootCountdown <= 0) { clearInterval(iv); flasherState = 'complete'; }
		}, 1000);
		return () => clearInterval(iv);
	});
	...
	const isPostFlashStep = $derived(
		...
		flasherState === 'booting' || flasherState === 'complete'
	);
...
{#if newToken}
	<div class="my-8 rounded-2xl border border-accent-primary/20 bg-accent-primary/[0.03] p-6 backdrop-blur-sm">
		<div class="flex items-start justify-between gap-4">
			<div class="min-w-0 flex-1">
				...
				<div class="mt-6 border-t border-separator-secondary pt-6">
					...
					{#if !webSerialSupported}
						<div class="rounded-xl border border-separator-secondary bg-background-primary p-4 text-center">
							<p class="text-sm text-label-secondary">
								Web Serial API is required. Use <strong>Chrome</strong>, <strong>Edge</strong>, or <strong>Opera</strong> desktop.
							</p>
						</div>

					{:else if flasherState === 'idle'}
						{#if flashError}
							<p class="mb-3 text-xs text-red-500">{flashError}</p>
						{/if}
						<div class="flex flex-wrap gap-3">
							...
							<input
								...
								class="min-w-32 flex-1 rounded-xl border border-separator-secondary bg-background-primary px-3 py-2 text-sm text-label-primary outline-none focus:border-accent-primary"
							/>
							<MainButton size="S" buttonStyle="primary" icon="mdi:cog" label="Build Firmware" onclick={handleFlashJob} />
						</div>
					...
					{:else}
						<div class="space-y-4 rounded-xl border border-separator-secondary bg-background-primary p-5">
							{#if flashError}
							...
							{:else if flasherState === 'ready'}
								<div class="flex flex-col items-center gap-3 py-2 text-center">
									...
									<p class="text-xs text-label-tertiary">Connect your ESP32-C3 via USB to flash it.</p>
								</div>
								<div class="flex justify-center">
									<MainButton buttonStyle="primary" size="M" icon="mdi:usb-port" label="Connect Device" onclick={handleConnect} />
								</div>
								...
								{#if flashChipInfo}
									<div class="grid grid-cols-2 gap-3 text-sm">
										<div>
											...
											<p class="font-display font-bold text-label-primary">{flashChipInfo.chipName}</p>
										</div>
										<div>
											<span class="text-xs text-label-tertiary">Description</span>
											<p class="font-display font-bold text-label-primary">{flashChipInfo.chipId}</p>
										</div>
									</div>
								{/if}
								<div class="flex justify-center gap-3">
									<MainButton buttonStyle="primary" size="M" icon="mdi:flash" label="Flash Firmware" onclick={handleFlash} />
									<MainButton buttonStyle="ghost" size="M" icon="mdi:close" label="Disconnect" onclick={handleDisconnect} />
								</div>

							{:else if flasherState === 'flashing'}
								<div>
									<div class="mb-2 flex items-center justify-between">
										<span class="font-display text-xs font-bold text-label-secondary">Writing firmware</span>
										<span class="font-display text-xs font-bold text-label-primary">{flashProgress}%</span>
									</div>
							...
							{:else if flasherState === 'error'}
								<div class="flex justify-center">
									<MainButton buttonStyle="secondary" size="M" icon="mdi:refresh" label="Start Over" onclick={resetFlasher} />
								</div>
							{/if}

							{#if flashLogs.length > 0}
								<div bind:this={logContainer} class="max-h-36 overflow-y-auto rounded-lg border border-separator-secondary bg-accent-primary p-3">
			...
			<button onclick={closeTokenAlert} class="shrink-0 text-label-tertiary hover:text-label-primary">
